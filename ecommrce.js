  ...
  const parentSection = document.getElementById('music-content')
      prodcuts.forEach(product => {
        ...
        button.addEventListener('click',(e)=>{
          ...
          axios.post('http://localhost:4000/cart',{productId:id})
          .then((res)=>{
            console.log(res)
            if(res.status === 200){
  ...
  matchingProductDetails.forEach(item=>{
    ...
    // console.log(typeof(quantity),typeof(price)) 
      ...
      cartQuantityButton.addEventListener('click',(e)=>{
        ...
        axios.post(`http://localhost:4000/cartdelete/${id}`)
        .then(res=>{
      ...
      button1.className = 'order-button';
      button1.type = 'button';
...
parentContainer.addEventListener("click", (e) => {  
  if (
        ...
        e.target.className == "remove-item"
      ) {
      ...
      if (e.target.className=='order-button'){
        ...
        axios.post('http://localhost:4000/orders', { productId: productId})
        .then(response => {
